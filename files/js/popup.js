...
utils_Modalwindow.init = function() {
	utils_Modalwindow._jParent = $("#modalwindow");
...
utils_Modalwindow.open = function() {
	...
	var h = utils_Modalwindow._jContent.height();
	var winH = view_PopupWindow.getHeight();
	var posi = (winH - h) * .5;
	utils_Modalwindow._jBg.css({ opacity : 0}).animate({ opacity : 1},200);
	utils_Modalwindow._jContent.css({ top : posi + 10, opacity : 0}).animate({ top : posi, opacity : 1},200);
};
utils_Modalwindow.close = function() {
	utils_Modalwindow._jBg.animate({ opacity : 0},200,null,function() {
		utils_Modalwindow._jParent.hide();
	});
};
...
view_page_Memo.prototype = $extend(view_Page.prototype,{
	...
	,set: function(data) {
		...
		if(dataArray == null) {
			dataArray = [{ id : 1, name : "Memo", value : "ここにテキストを入力してください"}];
			utils_Modalwindow.ask({ title : "はじめまして", text : "これはシンプルで最強のメモ帳です。"});
		}
